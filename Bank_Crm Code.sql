...
SELECT c.CustomerId, c.Surname, c.EstimatedSalary,c.Bank_DOJ
FROM customer c
JOIN bank_churn b ON c.CustomerId = b.CustomerId
WHERE EXTRACT(QUARTER FROM c.Bank_DOJ) = 4  
...
FROM customer ci LEFT JOIN ActiveAccounts aa ON ci.CustomerId = aa.CustomerId
GROUP BY Gender
...
FROM geographylocation g
INNER JOIN customer c ON g.GeographyID = c.GeographyID
...
WITH AverageGeographySalary AS (
    ...
    FROM customer c
    INNER JOIN geographylocation g ON c.GeographyID = g.GeographyID
...
JOIN geographylocation g ON c.GeographyID = g.GeographyID
WHERE b.Exited = 1
GROUP BY g.GeographyLocation
...
SELECT *
FROM bank_churn b join customer c on b.CustomerId = c.CustomerId
